...
export default function Edit({ maintain, maintainTypes }: Props) {
    const [photoPreview, setPhotoPreview] = useState<string | null>(
        maintain.photo ? `/storage/${maintain.photo}` : null
    );

    const { data, setData, post, processing, errors } = useForm({
        maintain_type_id: maintain.maintain_type_id.toString(),
        name: maintain.name,
    ...
    const handleSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        post(`/admin/maintains/${maintain.id}`);
    };

    return (
        <AdminLayout>
            <Head title="Edit Maintain" />
            ...
            <div className="container mx-auto px-4 py-8">
                <div className="flex items-center gap-4 mb-6">
                    ...
                    <h1 className="text-2xl font-bold text-gray-900">Edit Maintain</h1>
                </div>
